 ...
 */
std::vector<PCD, Eigen::aligned_allocator<PCD>> Load(const std::vector<std::string> &paths)
{
    std::vector<PCD, Eigen::aligned_allocator<PCD>> models;
    std::string extension = ".pcd";
    // Suppose the first argument is the actual test model
    for (const std::string &fname : paths)
    {
...
/* ---[ */
int Register(const std::vector<std::string> &paths)
{
    // Load data
    std::vector<PCD, Eigen::aligned_allocator<PCD>> data = Load(paths);
    ...
    for (size_t i = 1; i < data.size(); ++i)
    {
        ...
        PCL_INFO("Aligning %s (%d) with %s (%d).\n", data[i - 1].name.c_str(), source->points.size(), data[i].name.c_str(), target->points.size());
